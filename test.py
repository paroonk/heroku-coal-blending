...
def coal_optimize():
    ...
    def simulation(df, show_solver_log=False):
        ...
        cfb12_mix = {p: {d: sum(m.cfb12_ratio[d, s] * coal_df.loc[s, param_lookup_list[param_list.index(p)]] for s in supplier) / sum(m.cfb12_ratio[d, s] for s in supplier) for d in date_rng[1:]} for p in param_list}
        cfb3_mix = {p: {d: sum(m.cfb3_ratio[d, s] * coal_df.loc[s, param_lookup_list[param_list.index(p)]] for s in supplier) / sum(m.cfb3_ratio[d, s] for s in supplier) for d in date_rng[1:]} for p in param_list}
        ...
        cfb12_ash_flow = {d: -0.157 + 0.1464 * Qro12_norm - 0.1778 * cfb12_mix_norm['GCV'][d] + 0.7089 * cfb12_lime_flow_norm[d] + 0.423 * cfb12_mix_norm['%Ash'][d] - 0.056 * Excess_O2 + 0.1962 * Pa_ratio for d in date_rng[1:]}
        cfb12_boiler_eff = {
            d: 98.2894
            + 0.2862 * Qro12_norm
            + 5.453 * cfb12_mix_norm['GCV'][d]
            - 4.5807 * cfb12_mix_norm['TM'][d]
            - 17.9721 * cfb12_mix_norm['%H'][d]
            - 0.4126 * cfb12_mix_norm['%Ash'][d]
            - 2.7522 * cfb12_lime_flow_norm[d]
            - 2.9131 * T12[d]
            - 2.6225 * Excess_O2
            for d in date_rng[1:]
        }
        ...
        cfb3_ash_flow = {d: -0.157 + 0.1464 * Qro3_norm - 0.1778 * cfb3_mix_norm['GCV'][d] + 0.7089 * cfb3_lime_flow_norm[d] + 0.423 * cfb3_mix_norm['%Ash'][d] - 0.056 * Excess_O2 + 0.1962 * Pa_ratio for d in date_rng[1:]}
        cfb3_boiler_eff = {
            d: 98.2894
            + 0.2862 * Qro3_norm
            + 5.453 * cfb3_mix_norm['GCV'][d]
            - 4.5807 * cfb3_mix_norm['TM'][d]
            - 17.9721 * cfb3_mix_norm['%H'][d]
            - 0.4126 * cfb3_mix_norm['%Ash'][d]
            - 2.7522 * cfb3_lime_flow_norm[d]
            - 2.9131 * T3[d]
            - 2.6225 * Excess_O2
            for d in date_rng[1:]
        }
        ...
        fe2o3_cost = {d: cfb12_use_sum[d] * (sigmoid(cfb12_mix['%Fe2O3'][d], 355.34, 1, 40, 14.5)) + cfb3_use_sum[d] * (sigmoid(cfb3_mix['%Fe2O3'][d], 355.34, 1, 40, 14.5)) for d in date_rng[1:]}
        na2o_cost = {d: cfb12_use_sum[d] * (sigmoid(cfb12_mix['%Na2O'][d], 355.34, 1, 40, 5)) + cfb3_use_sum[d] * (sigmoid(cfb3_mix['%Na2O'][d], 355.34, 1, 40, 5)) for d in date_rng[1:]}
    ...
    def expand_schedule(df, show_solver_log=False):
        ...
        # calculate coal mixing parameters
        cfb12_mix = {p: {d: sum(cfb12_ratio[d][s] * coal_df.loc[s, param_lookup_list[param_list.index(p)]] for s in supplier) / sum(cfb12_ratio[d][s] for s in supplier) for d in date_rng[1:]} for p in param_list}
        cfb3_mix = {p: {d: sum(cfb3_ratio[d][s] * coal_df.loc[s, param_lookup_list[param_list.index(p)]] for s in supplier) / sum(cfb3_ratio[d][s] for s in supplier) for d in date_rng[1:]} for p in param_list}
